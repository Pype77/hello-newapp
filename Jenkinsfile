...
podTemplate(containers: [
      ...
      containerTemplate(name: 'kaniko', image: 'gcr.io/kaniko-project/executor:debug-v0.19.0', command: "/busybox/cat", ttyEnabled: true, shell: '/busybox/sh')
  ],
  volumes: [
    ...
    node(POD_LABEL) {
        ...
        stage('build') {
            container('kaniko') {
              ...
              sh "/kaniko/executor --force --dockerfile=Dockerfile --context=dir://. --destination=${appimage}:${apptag}"
            }
        } //end build
